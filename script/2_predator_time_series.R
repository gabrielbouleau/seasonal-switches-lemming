...
owlDensity <- fread("data_raw/owl_nest_lemming_density_gauthier.csv")[,-9] %>%
  ...
  dplyr::select(year = Year, Owl_C1, Nest_density_C1) %>% 
...
fox_repro <- fox[which(fox$Den_ID %in% Den_ID_C1 & fox$Species != "Red fox"),] %>%
  ...
  dplyr::select(Year, Den_ID, Status) %>% 
  ...
  dplyr::select(Year, N, R, NR, sample_size) %>% 
  mutate(NR_sum = sum(N, R)) %>% 
  add_column(fox_repro = ifelse(.$NR_sum > fox_threshold, 1, 0)) %>% 
  dplyr::select(Year, sample_size, NR_sum, fox_repro) %>% 
  mutate(Season = "S", period = "P1")
...
saveRDS(Predator, "data_clean/2_PredatorPresence.rds")
...
saveRDS(PredatorAbundance, "data_clean/2_PredatorAbundance.rds")
...
saveRDS(PredDensity, "data_clean/2_PredMeanDensity.rds")
